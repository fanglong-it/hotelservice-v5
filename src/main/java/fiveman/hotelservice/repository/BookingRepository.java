...
public interface BookingRepository extends JpaRepository<Booking, Long> {
        ...
        @Query(value = "select count(booking.id) from booking where booking.room_id > 0 and STR_TO_DATE(booking.actual_arrival_date, '%d/%m/%Y') between STR_TO_DATE(:startDate, '%d/%m/%Y') and STR_TO_DATE(:endDate, '%d/%m/%Y')", nativeQuery = true)
