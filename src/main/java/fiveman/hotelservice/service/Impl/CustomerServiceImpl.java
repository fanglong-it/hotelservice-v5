...
public class CustomerServiceImpl implements CustomerService {
    ...
    public Customer getPrimaryCustomerByBookingId(long booking_id) {
        ...
        if (customer == null) {
            throw new AppException(HttpStatus.NOT_FOUND.value(),
                    new CustomResponseObject(Common.GET_FAIL, "Not found id Customer by booking = " + booking_id));
        }
        return customer;
    }
    ...
    public List<Customer> saveCustomer(Customer customer) {
        if (customerRepository.existsById(customer.getId())) {
            throw new AppException(HttpStatus.ALREADY_REPORTED.value(),
                    new CustomResponseObject(Common.ADDING_FAIL, "Exist id =" + customer.getId()));
        }
    ...
    public Customer updateCustomer(Customer customer) {
        if (!customerRepository.existsById(customer.getId())) {
            throw new AppException(HttpStatus.NOT_FOUND.value(),
                    new CustomResponseObject(Common.UPDATE_FAIL, "Not found id =" + customer.getId()));
        }
        customerRepository.save(customer);
